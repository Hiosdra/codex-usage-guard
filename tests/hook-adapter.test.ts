...
describe("hook adapter", () => {
  ...
  test("maps allow, warning, block, and missing-data hook decisions", () =>
    withTestIsolation(
      [
        "PATH",
...
`,
        ...
        const configPath = process.env.CODEX_USAGE_GUARD_CONFIG;
        if (!configPath) throw new Error("test config path was not set");
        await Bun.write(
          configPath,
